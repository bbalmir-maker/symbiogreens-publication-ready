(function () {
  ...
  async function submitDecision(button) {
    ...
    const functionByStatus = {
      approved: "approve-application",
      rejected: "reject-application",
      needs_more_information: "request-more-info"
    };
    const functionName = functionByStatus[status];
    if (!functionName) return;
    button.disabled = true;
    setMessage("Submitting decision...");
    const body = { applicationId, decisionNotes };
    if (status === "needs_more_information") body.requestedInfoMessage = decisionNotes;
    const { data, error } = await window.SymbioBackend.invoke(functionName, body);
    button.disabled = false;
    if (error || data?.error) {
      const message = error?.message || data?.error || "Decision update failed.";
      console.error("[Admin] Application decision update failed:", message);
      setMessage(`Decision update failed: ${message}`);
      return;
    }
