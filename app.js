...
const I18N = {
  en: {
    home:'Home', about:'About', products:'Products', why:'Why Hydroponics', farmsProjects:'Farms & Projects', team:'Team', careers:'Careers', contact:'Contact', register:'Register as Buyer', buyerLogin:'Buyer Login', investors:'Investor & Partnerships',
...
function renderNav() {
  ...
  nav.innerHTML = `${items.map(([route,label]) => `<button class="nav-btn ${state.route===route?'active':''}" data-route="${route}">${esc(label)}</button>`).join('')}
    <a class="nav-btn" href="careers.html">${esc(t('careers'))}</a>
...
function renderRoute() {
  ...
  if (state.route === 'products') return `${internalPageHero('products', t('products'), t('productsPageTitle'), t('productsPageBody'), 'public/company/production/premium-product-assortment.png')}${productsPanel(false)}`;
  if (state.route === 'whyHydroponics') return `${internalPageHero('hydroponics', t('homeHydroEyebrow'), t('homeHydroTitle'), t('homeHydroSubtitle'), 'public/company/production/why-hydroponics-root-zone.png')}${whyHydroponicsPanel()}`;
...
function internalPageHero(kind, eyebrow, title, body, image) {
  return `<section class="sg-page-hero sg-page-hero--${esc(kind)}" style="--sg-page-hero-image:url('${esc(image)}')"><div class="sg-page-hero-copy"><div class="eyebrow">${esc(eyebrow)}</div><h1>${esc(title)}</h1><p>${esc(body)}</p></div></section>`;
}
...
function heroPanel() {
  const slides = [
    'public/company/production/homepage-hero-greenhouse.png',
    'public/company/production/home-symbiogreens-product-family.png',
    'public/company/production/home-symbiogreens-balponics-buyer-model.png',
    'public/company/production/greenhouse-interior-tower-production.png'
  ];
  return `<section class="homepage-hero sg-carousel-hero" aria-label="${esc(t('heroTitle'))}"><div class="sg-hero-slides" aria-hidden="true">${slides.map(src => `<div class="sg-hero-slide" style="background-image:url('${esc(src)}')"></div>`).join('')}</div><div class="homepage-hero-overlay"><div class="hero-copy-overlay"><div class="eyebrow">${esc(t('heroEyebrow'))}</div><h1>${esc(t('heroTitle'))}</h1><strong class="hero-subhead">${esc(t('heroSubhead'))}</strong><p>${esc(t('heroBody'))}</p><div class="hero-actions"><button class="primary-btn" data-route="account">Create Account / Apply for Access</button><button class="ghost-btn" data-action="startProductionSurvey">${esc(t('takeSurvey'))}</button><button class="ghost-btn" data-route="products">${esc(t('explore'))}</button><button class="ghost-btn" data-route="lasTerrenas">${esc(t('farmsProjectsCta'))}</button></div></div></div></section>`;
}
